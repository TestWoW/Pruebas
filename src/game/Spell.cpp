...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (target && target->IsInWorld())
    {
        ...
        if (non_caster_target)
        {
            ...
            if(!m_IsTriggeredSpell && !m_spellInfo->HasAttribute(SPELL_ATTR_EX2_IGNORE_LOS) && VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) && !m_caster->IsWithinLOSInMap(target, false)) //LOS HACK
...
bool Spell::FillCustomTargetMap(SpellEffectIndex i, UnitList &targetUnitMap)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        {
            UnitList tmpUnitMap;
            FillAreaTargets(tmpUnitMap, radius, PUSH_INHERITED_CENTER, SPELL_TARGETS_AOE_DAMAGE);
            if (!tmpUnitMap.empty())
            {
                for (UnitList::const_iterator itr = tmpUnitMap.begin(); itr != tmpUnitMap.end(); ++itr)
                {
        ...
        {
            UnitList tempTargetUnitMap;
            FillAreaTargets(tempTargetUnitMap, radius, PUSH_INHERITED_CENTER, SPELL_TARGETS_ALL);
            if (!tempTargetUnitMap.empty())
            {
                for (UnitList::const_iterator iter = tempTargetUnitMap.begin(); iter != tempTargetUnitMap.end(); ++iter)
                {
